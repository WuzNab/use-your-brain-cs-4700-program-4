...
//Direct set
//Hit_Points.Instance.SetFootballHP(2);

//Check if game over
//if (Hit_Points.IsGameOver) { /* stop spawns, etc. */ }

public class Hit_Points : MonoBehaviour
{
    [Header("Current HP")]
    public int Cone_Zombie_Hit_Points = 3;
    public int Football_Zombie_Hit_Points = 3;
    public int Balloon_Zombie_Hit_Points = 3;

    [Header("Overlay")]
    [SerializeField] GameObject Game_Over_Canvas;

    // Hit_Points.IsGameOver
...
void AfterChange()
    ...
    void SetLabel(string childName, int value)
    {
        ...
        if (!t) t = GameObject.Find(childName)?.transform;
